...
namespace PD
{
    class Outrage_Z : @base
    {
        ...
        public Outrage_Z(double Kp, double Tu, int point_proc, int start_time, int end_time)
        {
        ...
        public double[] calc(double[] a_c, double[] b_c)
        {
            ...
            for (int i = 0; i < point_proc_c; i++)
            {
                ...
                result[i] = koeff_Kp * ((start_time_c + srznach * i) - koeff_Tu * (1-Math.Exp(-(start_time_c + srznach * i)/ koeff_Tu))); // временная характеристика переходной функции звена с замедлением
            };
