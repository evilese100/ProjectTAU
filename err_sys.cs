...
namespace PD
{
    class err_sys:@base
    {
        ...
        public err_sys(double Kp, double Tu, int point_proc, int start_time, int end_time)
        {
        ...
        public double[] calc(double[] a_c, double[] b_c)
        {
            ...
            for (int i = 0; i < point_proc_c; i++)
            {
                ...
                result[i] = (koeff_Kp * (start_time_c + srznach * i) + koeff_Kp); // временная характеристика интегрального изодромного звена
            };
